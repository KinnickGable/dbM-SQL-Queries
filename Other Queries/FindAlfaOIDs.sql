...
-- 01/21/2013 bobh  - modified first 2 CodeSystem Table queries for titles and count of codes and use( only effective in releases >4.4 with Popularity enabled)
...
SELECT  COUNT (*) [Codesystems with Alpha Oids]
FROM         Vocabulary.CodeSystem
WHERE CodeSystem LIKE '%[_]%' 

SELECT  c.CodeSystem,c.CodeSystemName,c.OriginalCodeSystem, COUNT(csc.CSCID) as [Number of Codes in CodeSystem],sum(csc.Popularity)as [Sum of Popularity]
FROM         Vocabulary.CodeSystem c 
join dbmVCDRData.Vocabulary.CodeSystemConcept csc
on c.CodeSystemID=csc.CodeSystemID
WHERE CodeSystem LIKE '%[_]%'
group by  c.CodeSystem,c.CodeSystemName,c.OriginalCodeSystem
-----------------------------------------------------------
