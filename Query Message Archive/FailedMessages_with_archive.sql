...
----------------------------- Events\Extended properties definition
		...
		INSERT INTO #EventLog(Event_ID, OccurrenceTime, LogLevel, [Message])
		SELECT Event_ID, OccurrenceTime, LogLevel, [Message]
		FROM #DILEvent AS DL 
		...
		INSERT INTO #ExtendedProperties(Event_ID, [Name], [Value])
		SELECT B.Event_ID, B.Name, LTRIM(RTRIM(B.[Value]))
		FROM #EventLog AS C
