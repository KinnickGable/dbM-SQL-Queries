...
Description: Returns amount of rows loaded to PatientNamePart in last 24 hours and contains symbol "?" in the value. 
*/
...
begin	
	...
	SELECT @MsgCount = @MsgCount + count(PatientNamePartID)
	FROM  MessageWrapper.ControlAct (nolock)
		  INNER JOIN PatientAdministration.PatientNamePart (nolock)
			ON ControlAct.ControlActID = PatientNamePart.ControlActID
	WHERE ControlAct.ControlActID > @minControlActID 
		  and ControlAct.ControlActID < (@minControlActID + @chunkSize + 1) 
		  and PatientNamePart.[Value] like '%?%'

	set @minControlActID = @minControlActID + @chunkSize
end

select @MsgCount as PatientNamePart
