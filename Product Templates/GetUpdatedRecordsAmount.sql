...
/* by AlexT 
added where condition by MessageWrapper.ObsoleteMetadata.ObsoleteMetadataID according to Oz L. recommendation 
use Index Seek instead table scan
*/
...
DECLARE @Min_ObsoleteMetadataID bigint
DECLARE @Max_ObsoleteMetadataID bigint



SELECT  @dtEnd = '2010-03-03',--GETDATE(),
		@dtStart = DATEADD(dd,-1,@dtEnd)

--select @dtStart,@dtEnd
/*changed by AlexT*/
SELECT 
	@Min_ObsoleteMetadataID = MIN(ObsoleteMetadataID),
	@Max_ObsoleteMetadataID = MAX(ObsoleteMetadataID)
FROM MessageWrapper.ObsoleteMetadata
WHERE dbmObsoleteTime >= @dtStart
and dbmObsoleteTime < @dtEnd

--select @Min_ObsoleteMetadataID,@Max_ObsoleteMetadataID
...
FROM MessageWrapper.ObsoleteMetadata ObsoleteMetadata
...
WHERE  ObsoleteMetadata.ObsoleteMetadataID >= @Min_ObsoleteMetadataID
AND ObsoleteMetadata.ObsoleteMetadataID < @Max_ObsoleteMetadataID

--AND ObsoleteMetadata.dbmObsoleteTime >= @dtStart
